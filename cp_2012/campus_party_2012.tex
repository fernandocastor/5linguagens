...
\vspace{0.2cm}
\begin{center}
\Huge{Primeira Linguagem de Programação para Aprender}
\end{center}
\end{frame}
...
\begin{itemize}
\item Java ficou \tred{verborrágica} e \tred{complexa}\vspace{0.1cm}\pause
\\
...
\item Muitos desenvolvedores \textbf{migraram} para outras linguagens
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
% Scala! Twitter, multiparadigma, compatibilidade retroativa com Java. As principais funcionalidades que se pretende incluir em Java 8 já estão em Scala desde as primeiras versões. Isso sugere que a integração entre entre essas funcionalidades e o restante da linguagem é mais simples em Scala. Além disso, Scala inclui construções para programação paralela e concorrente que não são nem mencionadas para Java (não da forma simples como existem em Scala).
% Uma comparação mais detalhada está disponível em: http://www.infoq.com/articles/java-8-vs-scala
