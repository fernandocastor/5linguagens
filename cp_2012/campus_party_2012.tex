...
\item E já fornecem estatísticas\vspace{0.1cm}
\end{itemize}
\end{itemize}
\pause
\begin{block}{}
\textbf{Importante}: não representam a \tred{indústria} como um todo
\end{block}
\vspace{0.2cm}
...
\begin{column}[l]{5cm}
{\scriptsize
Os oficiais, com base em número de repositórios únicos:
\begin{enumerate}
\item JavaScript (20\%)
\item Ruby (14\%)
\item Python (9\%)
...
% String de busca usada no Github: 
%BASH CSH TCSH ZSH KSH ActionScript Ada Apex Arc Arduino ASP Assembly Augeas AutoHotkey Boo Bro C C# C++ Ceylon Clojure CoffeeScript ColdFusion Common Lisp Coq D Dart DCPU-16 ASM Delphi Dylan eC Eiffel Elixir Emacs Lisp Erlang F# Factor Fancy Fantom FORTRAN Go Gosu Groovy Haskell HaXe Io Ioke Java JavaScript Julia Kotlin Logtalk Lua Matlab Max/MSP Mirah Nemerle Nimrod Nu Objective-C Objective-J OCaml ooc Opa OpenEdge ABL Parrot Perl PHP PowerShell Prolog Puppet Pure Data Python R Racket Rebol Ruby Rust Scala Scheme Scilab Self Shell Smalltalk Standard ML SuperCollider Tcl Turing Vala Verilog VHDL VimL Visual Basic XQuery
% Basicamente, peguei a lista completa de linguagens que aparece em http://github.com/languages e acrescentei BASH, CSH, TCSH, ZSH e KSH
% Com Repositories in Any Language.

% Comparação: o que se fala (em termos de número de tags no StackOverflow) contra o que se usa (de acordo com o ranking do Github). O Resultado foi uma correlação alta.  
